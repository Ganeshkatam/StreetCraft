...
import { generateCampaignPack, CampaignGenerationInput } from '../src/engine/campaignEngine';
...
for (const sc of TEST_SCENARIOS) {
  ...
  if (!g.headline || !g.body || !g.ctaType) {
    console.error('  FAIL: Google Business output missing headline, body or ctaType');
    continue;
  }

  // 3. Instagram Proof Check
  const ig = outputs.instagram;
  if (!ig.reelHook || !ig.caption || !ig.storyFrames || ig.storyFrames.length === 0) {
    console.error('  FAIL: Instagram output missing reelHook, caption or storyFrames');
    continue;
  }

  // 4. WhatsApp Proof Check
  const wa = outputs.whatsapp;
  if (!wa.broadcastMessage || !wa.cta) {
    console.error('  FAIL: WhatsApp output missing broadcastMessage or cta');
    continue;
  }

  // 5. In-Store Poster Check
  const poster = outputs.poster;
  if (!poster || !poster.headline || !poster.body || !poster.cta) {
    console.error('  FAIL: Poster output missing headline, body, or cta');
    continue;
  }
  ...
  console.log(`        - WhatsApp Snippet: "${wa.broadcastMessage.substring(0, 80)}..."`);
  console.log(`        - Poster Headline: "${poster.headline}"`);
  console.log(`        - Cross-Channel Consistency: "${sc.input.offer.title}" preserved across all formats.\n`);
