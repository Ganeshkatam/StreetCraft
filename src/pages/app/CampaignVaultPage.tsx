        ...
        {(['ALL', 'PUBLISHED', 'COMPLETED', 'ARCHIVED'] as const).map((tab) => (
          <button
            ...
            style={{
              ...
              background: filter === tab ? 'var(--bg-surface-elevated)' : 'transparent',
              border: filter === tab ? '1px solid var(--border-medium)' : '1px solid transparent',
              padding: '6px 14px',
          ...
          {filteredCampaigns.map((item) => {
            ...
            return (
              <div key={item.campaign.id} className="card" style={{ padding: '24px' }}>
                ...
                {/* Operator Note */}
                <div style={{ background: 'var(--bg-input)', padding: '12px 16px', borderRadius: 'var(--radius-xs)', border: '1px solid var(--border-subtle)', marginBottom: isExpanded ? '16px' : 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                  <div style={{ flex: 1, minWidth: '240px' }}>
                    <small style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', fontSize: '10.5px' }}>WALK-IN NOTE</small>
                    {isEditingNotes ? (
                      ...
                      <div style={{ fontSize: '13px', color: item.campaign.performanceNotes ? '#FFFFFF' : 'var(--text-muted)', marginTop: '2px' }}>
                ...
                {/* Expanded 4 Channels */}
                {isExpanded && (
                  <div className="proofs-grid-2x2" style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid var(--border-subtle)' }}>
                    <ChannelCard
